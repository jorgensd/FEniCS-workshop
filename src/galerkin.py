...
def plot_interval_basis_functions(N:int, degree:int, g):
    ...
    reference_points = basix.create_lattice(
        basix.CellType.interval, 100, basix.LatticeType.equispaced, exterior=True
    )
    ...
    func_plotter.add_lines(np.vstack([_x, g(_x), np.zeros_like(_x)]).T, connected=True,
                       label=r"$g$", color="red", width=3)
